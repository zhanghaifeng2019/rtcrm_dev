...
<div class="search-condition-wrap formset">
	...
	<div class="search-condition-line">
		<dl class="search-condition-list search-condition-list--2column">
		<dt>チームリスト</dt>
		<dd>
 ...
 <div class="search-condition-line">
	<dl class="search-condition-list search-condition-list--date">
		<dt>最新連絡日</dt>

		<dd><input type="date" value="<?php if(isset($_SESSION["contact_date_from"])){echo $_SESSION["contact_date_from"];}?>" name="contact_date_from"></dd>
		<dd>-</dd>
		<dd><input type="date" value="<?php if(isset($_SESSION["contact_date_end"])){echo $_SESSION["contact_date_end"];}?>" name="contact_date_end"></dd>
	</dl>
	<dl class="search-condition-list search-condition-list--date">
			<dt>キーワード</dt>
			<dd style="width:calc( 100% - 160px )"><input type="text" class="input" name="keywords" value="<?php if(isset($_SESSION['keywords'])){echo $_SESSION['keywords'];} ?>"></dd>
	</dl>
</div>
			...
			<div class="search-record search-record--tmsearch">
				...
				<div class="search-record-table">
					...
					<dl>
						...
						<?php
							if(empty($item->timer_date)){
								echo date("Y-m-d", strtotime( $item->CLcreated));
							}
							else{
								echo date("Y-m-d", strtotime( $item->timer_date));
							}
						 ?>
						</span></dd>
					</dl>
...
$( "#sortBox" ).change(function() {
	if($( "#sortBox" ).val()==1){
		window.location.replace("<?=base_url()?>TMAll/sorting/timerAsc");
	}
	else if($( "#sortBox" ).val()==2){
		window.location.replace("<?=base_url()?>TMAll/sorting/timerDesc");
	}
	else if($( "#sortBox" ).val()==3){
		window.location.replace("<?=base_url()?>TMAll/sorting/contact_dateAsc");
	}
	else if($( "#sortBox" ).val()==4){
		window.location.replace("<?=base_url()?>TMAll/sorting/contact_dateDesc");
	}
});
...
$(window).on('load', function (){
    ...
    ?>
      if( "<?=$_SESSION['staff_id']?>" == value['id'] ){
        $('#staffSelect1').append("<option value="+value['id']+" selected>"+value['name']+"</option>");
