  ...
  public function __construct()
  {
       ...
       if(!isset($this->session->name)){
        ...
        public function index()
        {
          ...
          $this->session->set_userdata('cancelPage',"TMAll");
                  ...
                  $config=array();
                  $config['base_url'] =base_url()."/TMAll/index";
                  $config['per_page']=10;
                  // $config['use_page_numbers'] = TRUE;
                  $data['totalRows']=$this->Result_model->getAllRecord()[0]->num;
                  // $page_num=($this->uri->segment(3)) ? $this->uri->segment(3) : 1;


                  $config['total_rows']=$data['totalRows'];
                  $config["num_links"] = 2;
                  $data['table']= $this->Result_model->getAllList($config["per_page"],$param);

                  // $allId=$this->Result_model->getAllList("all",0);
                  // $idArr=[];
                  //
                  // foreach ($allId as $key => $value) {
                  //   array_push($idArr,$value->id) ;
                  // }
                  //     $this->session->set_userdata('idArr',$idArr);;
          ...
          public function post()
          {
            ...
            if(isset($_POST["contact_date_from"])){
              $this->session->set_userdata('contact_date_from',$_POST['contact_date_from']);
            }
            if(isset($_POST["contact_date_end"])){
              $this->session->set_userdata('contact_date_end',$_POST['contact_date_end']);
            }
          ...
          public function outputCsv(){
            ...
            $query = $this->db->query($this->Result_model->getRecordTMAllCsv());
            $priority=['最高','高','中','低'];
            $data[0]['id']="ID";
            $data[0]['priority']='優先';
