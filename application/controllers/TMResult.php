<?php
class TMResult extends CI_Controller {
       ...
       if(!isset($this->session->name)){
        ...
        public function index()
        {
            ...
            if(!isset($_SESSION['PDSVName']) && !isset($_SESSION['Phone']) && !isset($_SESSION['companyName']) && !isset($_SESSION['domain']))
              {
                ...
                $data['type']="mypage";
                $config=array();
                $config['base_url'] =base_url()."/TMResult/index";
                $config['per_page']=10;

                $data['totalRows']=count($this->Result_model->getMyRecord());

                $config['total_rows']=$data['totalRows'];
                $config["num_links"] = 2;
                $data['table']= $this->Result_model->getMyList($config["per_page"],$param);
                ...
                $this->load->view('common/footer');
                  ...
                  $data['totalRows']=count($this->Result_model->getRecord());

                  $config['total_rows']=$data['totalRows'];
                  $config["num_links"] = 2;
                  $data['table']= $this->Result_model->getList($config["per_page"],$param);
          ...
          public function outputCsv(){
            ...
            $query = $this->db->query($this->Result_model->getRecordCsv());
            $priority=['最高','高','中','低'];
            $data[0]['id']="ID";
            $data[0]['priority']='優先';
